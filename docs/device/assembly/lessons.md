...
- Measure continuity with a multimeter before powering up the circuit.
- Validate the circuit on a breadboard before soldering on a protoboard.
...
- Choose the right tools for the job to prevent frustration, inaccuracies, unreliable performance, and wasting time.
- Avoid cheap or very old tools if possible.
- Use precision tools for precision work.
...
- The recording LED turns on briefly when turning on the device, even if the record button is not pressed. Find out why.
...
- Handles' short sides should be wider, at least 2-3mm more, but leaving the holes on roughly the same place. Alternatively, they should have extensions to the sides to pass the screws through there. Maybe self-tapping screws can be used instead of machine screws.
...
- Not having bosses on the the lid (or other similar mechanisms) may be a problem, as the board can move around if the glue to the base fails.
