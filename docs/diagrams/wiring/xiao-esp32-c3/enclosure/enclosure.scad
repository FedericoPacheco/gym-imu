		...
		union() {
			...
			for (y_offset = [-HANDLE_INSERT_Y_OFFSET, HANDLE_INSERT_Y_OFFSET]) {
				translate([0, y_offset])
					rounded_rectangle_2d([HANDLE_OUTER_DEPTH_X, HANDLE_MOUNT_PAD_Y_SIZE], HANDLE_CORNER_RADIUS / 2);
			}
		}
	...
	difference() {
		...
		for (y_offset = [-HANDLE_INSERT_Y_OFFSET, HANDLE_INSERT_Y_OFFSET]) {
			translate([0, y_offset, HANDLE_THICKNESS_Z / 2])
				rotate([0, 90, 0])
					cylinder(d = HANDLE_SCREW_PASS_DIAMETER, h = HANDLE_OUTER_DEPTH_X + (2 * EPS), center = true);
		}
	}
}
