...
* The board will be secured to the base using two large and internal rectangular prism supports that run along the y axis, one per side, rather than isolated corner posts. These will have holes for heat-set inserts and will pass through the whole base, being visible from the outside. The screws that hold the lid will also go through the board and into the same inserts, so the mounting holes must be precisely aligned. The lid will not have prisms nor posts.
